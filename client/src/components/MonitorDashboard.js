...
const MonitorDashboard = ({ token, serverUrl, userId, onLogout, isEmbedded, onSessionChange, onOpenSessionReports, onOpenGeneralReports }) => { // Added isEmbedded <!-- id: 402 -->
    ...
    useEffect(() => {
        ...
        socket.on('lab-initial-state', ({ activeStudents, allowedStudents }) => {
            ...
            // FIX: Initialize raisedHands based on raiseHand flag in activeStudents
            const currentlyRaised = activeStudents.filter(s => s.raiseHand).map(s => s.username);
            setRaisedHands(currentlyRaised);
        });
    ...
    if (!sessionId || isCreatingSession) {
        return (
            <div style={{ width: '100%', height: '100%', background: 'transparent', color: '#e2e8f0', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '60px', fontFamily: 'Inter', padding: '40px' }}>
                {/* LEFT SIDE: REPORTS CARDS (Hidden in embedded mode to center form perfectly) */}
                {!isEmbedded && (
                        ...
                        {Object.entries(students).filter(([username, s]) => username.toLowerCase().includes(searchQuery.toLowerCase())).map(([username, s]) => {
                            ...
                            return (
                                <div key={username} onClick={() => handleSelectStudent(username)}
                                    style={{
                                        padding: '14px', marginBottom: '10px', borderRadius: '14px', cursor: 'pointer',
                                        background: isSelected ? 'rgba(99, 102, 241, 0.15)' : 'rgba(255, 255, 255, 0.02)',
                                        backdropFilter: 'blur(10px)',
                                        border: isRaised ? '2px solid #ef4444' : (isSelected ? '1px solid rgba(99, 102, 241, 0.4)' : '1px solid rgba(255, 255, 255, 0.05)'),
                                        display: 'flex', alignItems: 'center', gap: '14px',
                                        transition: 'all 0.3s cubic-bezier(0.4, 0, 0.2, 1)',
                                        position: 'relative', overflow: 'hidden',
                                        boxShadow: isRaised ? '0 0 15px rgba(239, 68, 68, 0.2)' : 'none',
                                        animation: isRaised ? 'card-pulse-red 2.5s infinite' : 'none'
                                    }}>
                                    ...
                                    <div style={{ position: 'relative' }}>
                                        <div style={{
                                            ...
                                            background: isSelected ? 'linear-gradient(135deg, #6366f1, #4f46e5)' : '#1e293b',
                                        ...
                                        <div style={{
                                            ...
                                            boxShadow: isActive ? `0 0 10px ${statusColor(s.status)}` : 'none',
                                            animation: isActive ? 'neonPulse StudentHighlight 2s infinite' : 'none'
                                        }}></div>
                                    </div>

                                    <div style={{ flex: 1, minWidth: 0 }}>
                                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '4px' }}>
                                            <div style={{ fontSize: '13.5px', color: isSelected ? '#fff' : '#f1f5f9', fontWeight: '700', textOverflow: 'ellipsis', overflow: 'hidden', whiteSpace: 'nowrap' }}>
                                                {username}
                                            </div>
                                            {isRaised && (
                                                <span style={{
                                                    fontSize: '14px', background: '#ef4444', color: '#fff',
                                        ...
                                        <div style={{ display: 'flex', flexWrap: 'wrap', gap: '8px', alignItems: 'center' }}>
                                            <div style={{
                                                fontSize: '10px', color: s.tabSwitchCount > 3 ? '#ef4444' : '#94a3b8',
                                                fontWeight: '700', background: 'rgba(0,0,0,0.2)', padding: '2px 6px', borderRadius: '4px'
                                            }}>
                                                📑 {s.tabSwitchCount || 0}
                                            </div>
                                            <div style={{
                                                fontSize: '10px', color: s.pasteCount > 5 ? '#fbbf24' : '#94a3b8',
                                                fontWeight: '700', background: 'rgba(0,0,0,0.2)', padding: '2px 6px', borderRadius: '4px'
                                            }}>
                                                📋 {s.pasteCount || 0}
                                            </div>
                                            <div style={{
                                                fontSize: '10px', color: s.attentionScore < 70 ? '#ef4444' : '#4ade80',
