...
const AIPanel = ({ token, code, fileName, language, editorContext, dirtyFiles, onApplyCode, onRunCommand, onAgentWorkspaceAction, targetAgentId = 'groq-assistant', onOpenSettings, userId }) => {
    ...
    useEffect(() => {
        // Electron panels can be mounted while their width animation is still
        // running. Focusing after that frame guarantees the composer remains
        // keyboard-ready every time the AI pane is opened or switched.
        const focusTimer = window.setTimeout(() => composerInputRef.current?.focus(), 180);
