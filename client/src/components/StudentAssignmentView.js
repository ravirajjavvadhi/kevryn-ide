...
const StudentAssignmentView = ({ 
    ...
    const [showLabArchive, setShowLabArchive] = useState(false);
    const [labArchive, setLabArchive] = useState([]);
    const [selectedArchiveFile, setSelectedArchiveFile] = useState(null);
    const [labArchiveQuery, setLabArchiveQuery] = useState('');
    ...
    const openLabArchive = async () => {
        setShowLabArchive(true); setSelectedArchiveFile(null); setLabArchiveQuery('');
        try { setLabArchive((await api.get('/lab/my-session-files')).data?.archive || []); }
        catch (_) { setLabArchive([]); }
    };
    ...
    return (
        <div style={{ position: 'relative', width: '100vw', height: '100vh', overflow: 'hidden' }}>
            ...
            {showLabArchive && <div className="student-profile-modal"><section style={{ maxWidth: 860 }}><header><div><span><FaHistory /> My lab files</span><p>Only code saved during your supervised lab sessions is shown here.</p></div><button onClick={() => setShowLabArchive(false)}><FaTimesCircle /></button></header><input value={labArchiveQuery} onChange={event => setLabArchiveQuery(event.target.value)} placeholder="Search course, session, or file…" style={{ width: '100%', boxSizing: 'border-box', marginBottom: 12, padding: 10, background: '#0b1220', color: '#fff', border: '1px solid rgba(148,163,184,.25)', borderRadius: 7 }} /><div style={{ display: 'grid', gridTemplateColumns: selectedArchiveFile ? 'minmax(240px, .8fr) minmax(0, 1.2fr)' : '1fr', gap: 14, maxHeight: '58vh', overflow: 'hidden' }}><div style={{ overflowY: 'auto', border: '1px solid rgba(148,163,184,.16)', borderRadius: 10 }}>{labArchive.length ? labArchive.map(session => ({ ...session, files: session.files.filter(file => `${session.subject} ${session.sessionName} ${file.path}`.toLowerCase().includes(labArchiveQuery.toLowerCase())) })).filter(session => session.files.length).map(session => <div key={session.sessionId} style={{ padding: 12, borderBottom: '1px solid rgba(148,163,184,.12)' }}><b style={{ fontSize: 13 }}>{session.sessionName}</b><small style={{ display: 'block', color: '#94a3b8', margin: '4px 0 8px' }}>{session.subject} · {session.date ? new Date(session.date).toLocaleDateString() : ''}</small>{session.files.map(file => <button key={`${session.sessionId}:${file.path}`} onClick={() => setSelectedArchiveFile({ ...file, session })} style={{ width: '100%', textAlign: 'left', border: 0, background: selectedArchiveFile?.path === file.path && selectedArchiveFile?.session?.sessionId === session.sessionId ? 'rgba(99,102,241,.18)' : 'transparent', color: '#dbeafe', borderRadius: 5, padding: '6px 7px', cursor: 'pointer' }}><FaCode size={10} /> {file.path}<small style={{ float: 'right', color: '#94a3b8' }}>{file.language || 'text'}</small></button>)}</div>) : <p style={{ padding: 20, color: '#94a3b8' }}>No supervised lab files have been saved yet.</p>}</div>{selectedArchiveFile && <div style={{ overflow: 'hidden', display: 'flex', flexDirection: 'column', border: '1px solid rgba(148,163,184,.16)', borderRadius: 10 }}><div style={{ padding: '10px 12px', color: '#c4b5fd', fontSize: 12, fontWeight: 700 }}>{selectedArchiveFile.path}<small style={{ display: 'block', color: '#94a3b8', fontWeight: 400, marginTop: 3 }}>{selectedArchiveFile.language || 'text'} · Updated {selectedArchiveFile.updatedAt ? new Date(selectedArchiveFile.updatedAt).toLocaleString() : '—'}</small></div><pre style={{ margin: 0, padding: 14, overflow: 'auto', background: '#0b1220', color: '#e2e8f0', flex: 1, fontSize: 12 }}>{selectedArchiveFile.code || '// This file was empty when saved.'}</pre></div>}</div><footer><button className="primary" onClick={() => setShowLabArchive(false)}>Done</button></footer></section></div>}
